...
def publish(request):
    ...
    views={}
    views['stories'] = Story.objects.all()
...
def category_detail(request, name):
    ...
    if current_user.is_authenticated:
        ...
        following_usernames = []
    return render(request, 'test2.html', {'stories': story, 'name': name, 'category': category, 'current_user': current_user, 'following_usernames': following_usernames})
...
def profile(request, username):
    ...
    views['user'] = user
    views['stories'] = Story.objects.filter(user=user)
...
def readmore(request,id):
    ...
    # Calculate the average rating for the stories
    ...
    return render(request, 'readmore.html', {'stories': story, 'user_ratings': user_ratings, 'category': category, 'average_rating': average_rating, 'current_user': current_user, 'following_usernames': following_usernames})
...
def submit_rating(request):
    if request.method == 'POST':
        ...
        # Check if the user has already rated the stories
        try:
            rating = Rating.objects.get(user=user, story=story_obj)
            rating.value = rating_value
            rating.save()
        except Rating.DoesNotExist:
            # If the user has not rated the stories, create a new rating
            rating = Rating.objects.create(user=user, story=story_obj, value=rating_value)

        # Recalculate average rating for the stories
